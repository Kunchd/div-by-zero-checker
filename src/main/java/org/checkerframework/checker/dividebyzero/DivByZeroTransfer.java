...
public class DivByZeroTransfer extends CFTransfer {
    ...
    private AnnotationMirror refineLhsOfComparison(
        ...
        switch (operator) {
            case EQ:
                if (equal(rhs, reflect(ZeroInt.class))) {
                    return reflect(ZeroInt.class);
                }
                else if (equal(rhs, reflect(NonZeroInt.class))) {
                    return reflect(NonZeroInt.class);
                }
                return glb(lhs, rhs);
            case NE:
                if (equal(rhs, reflect(ZeroInt.class))) {
    ...
    private AnnotationMirror arithmeticTransfer(
        ...
        switch (operator) {
            ...
            case MINUS:
                if (equal(lhs, top())) {
                    return top();
                } else if (equal(lhs, reflect(NonZeroInt.class))) {
                    if (equal(rhs, reflect(ZeroInt.class))) {
                        return lhs;
                    }
                    return top();
                } else if (equal(lhs, reflect(ZeroInt.class))) {
                    return rhs;
                }
                break;
            case TIMES:
                if (equal(lhs, reflect(ZeroInt.class)) || equal(rhs, reflect(ZeroInt.class))) {
                  return reflect(ZeroInt.class);
                }
            ...
            case MOD:
            case DIVIDE:
                if (equal(rhs, reflect(NonZeroInt.class))) {
                  if (equal(lhs, reflect(ZeroInt.class))) {
                    return reflect(ZeroInt.class);
                  }
                    return top();
                }
